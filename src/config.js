...
export function assignHiddenTraits(players) {
  return players.map(p => {
    const count = Math.random() < 0.01 ? 2 : Math.random() < 0.03 ? 1 : 0;
...
export function getTIRank(score) {
  if (score >= 92) return { id:'champion', title:'🎖️ TI 冠军', desc:'不朽盾上刻下了你的名字。', color:'text-yellow-400', bg:'from-yellow-600 to-amber-700' };
  if (score >= 90) return { id:'finalist', title:'🥈 TI 决赛', desc:'只差一步就能举起神盾。', color:'text-slate-300', bg:'from-slate-600 to-slate-800' };
  if (score >= 84) return { id:'top4',    title:'🥉 TI 四强', desc:'世界前四，已是无数人仰望的高度。', color:'text-amber-400', bg:'from-amber-700 to-yellow-800' };
  if (score >= 78) return { id:'top8',    title:'📜 TI 八强', desc:'淘汰赛的聚光灯下，你证明了自己的眼光。', color:'text-purple-400', bg:'from-purple-700 to-indigo-800' };
  if (score >= 72) return { id:'group',   title:'📜 小组赛',   desc:'正赛的门槛已经跨过。', color:'text-blue-400', bg:'from-blue-700 to-cyan-800' };
